...
public class ClassUtil {
     ...
     * 
     * <p>
     * 对于非数组的类型，该方法等效于 Class.getName() 方法。<br>
     * 注意，该方法所返回的数组类名只能用于显示给人看，不能用于 Class.forName 操作。
     * </p>
     *
     * @return 用于显示的直观类名，如果原类名为空或非法，则返回 null
     */
     ...
     *
     * @param className 类名
     * @param processInnerClass 是否将内联类分隔符 '$' 转换成 '.'
     *
     * @return 直观的类名，或 null
     */
    ...
    /**
     * 取得指定对象所属的类的短类名，不包括package名。此方法可以正确显示数组和内联类的名称。
     * 
     * 例如：
     * <pre>
     *  ClassUtil.getShortClassNameForObject(Boolean.TRUE) = "Boolean" 
     *  ClassUtil.getShortClassNameForObject(new Boolean[10]) = "Boolean[]" 
     *  ClassUtil.getShortClassNameForObject(new int[1][2]) = "int[][]"
     * </pre>
     *
     * @param object 要查看的对象
     * @return 短类名，如果对象为 null ，则返回 null
     */
    ...
    /**
     * 取得指定对象所属的类的package名。对于数组，此方法返回的是数组元素类型的package名。
     *
     * @param object 要查看的对象
     * @return package名，如果对象为 null ，则返回 null
     */
    ...
    //XXX:取得类名和package名的resource名的方法。
     ...
     * @return 指定对象所属类的资源名，如果对象为空，则返回null
     */
     ...
     *
     * @param clazz 要显示类名的类
     * @return 指定类的资源名，如果指定类为空，则返回null
     */
    @SuppressWarnings("rawtypes")
     ...
     * @return 指定类名对应的资源名，如果指定类名为空，则返回null
     */
    ...
    /**
     * 取得指定对象所属的类的package名的资源名。对于数组，此方法返回的是数组元素类型的package名。
     * 
     * @param object 要查看的对象
     * @return package名，如果对象为 null ，则返回 null
     */
    ...
    /**
     * 取得指定类的package名的资源名。对于数组，此方法返回的是数组元素类型的package名。
     * 
     * @param clazz 要查看的类
     * @return package名，如果类为 null ，则返回 null
     */
    @SuppressWarnings("rawtypes")
    ...
    /**
     * 取得指定类名的package名的资源名。对于数组，此方法返回的是数组元素类型的package名。
     * 
     * @param className 要查看的类名
     * @return package名，如果类名为空，则返回 null
     */
    ...
    //XXX:取得类的信息，如父类, 接口, 数组的维数等。

    /**
     * 取得指定维数的 Array类.
     *
     * @param componentType 数组的基类
     * @param dimension 维数，如果小于 0 则看作 0
     * @return 如果维数为0, 则返回基类本身, 否则返回数组类，如果数组的基类为 null ，则返回 null
     */
    @SuppressWarnings("rawtypes")
     ...
     *
     * @param type 要查找的类
     * @return 如果是数组, 则返回数组元素的类型, 否则返回 null
     */
    @SuppressWarnings("rawtypes")
     ...
     *
     * @param clazz 要查找的类
     * @return 数组的维数. 如果不是数组, 则返回 0 ，如果数组为 null ，是返回 -1
     */
    @SuppressWarnings("rawtypes")
     ...
     * ClassUtil.getSuperclasses(java.util.ArrayList[][].class) 返回以下列表：
     * 
     * <ol>
     * <li>java.util.AbstractList[][]</li>
     * <li>java.util.AbstractCollection[][]</li>
     ...
     * </ol>
     * 
     * 注意，原子类型及其数组，将被转换成相应的包装类来处理。 例如： ClassUtil.getSuperclasses(int[][].class)
     * 返回以下列表：
     * 
     * <ol>
     ...
     *
     * @param clazz 要查找的类
     * @return 所有父类的列表，如果指定类为 null ，则返回 null
     */
    @SuppressWarnings("rawtypes")
     ...
     * ClassUtil.getInterfaces(java.util.ArrayList[][].class) 返回以下列表：
     * 
     * <ol>
     * <li>java.util.List[][]</li>
     * <li>java.util.Collection[][]</li>
     ...
     *
     * @param clazz 要查找的类
     * @return 所有接口的列表，如果指定类为 null ，则返回 null
     */
    @SuppressWarnings("rawtypes")
     ...
     * 
     * <ol>
     * <li>如果目标类 clazz 为 null ，总是返回 false。</li>
     * <li>如果参数类型 fromClass 为 null ，并且目标类型 clazz 为非原子类型，则返回true。 因为 null 可以被赋给任何引用类型。</li>
     * <li>调用 Class.isAssignableFrom 方法来确定目标类 clazz 是否和参数类fromClass 相同或是其父类、接口，如果是，则返回 true。</li>
     * <li>
     * 如果目标类型 clazz 为原子类型，参数类型 fromClass 可以是任何能扩展成该目标类型的原子类型及其包装类。 例如， clazz 为
     * long ，那么参数类型可以是 byte、
     * short、int、long、char 及其包装类
     * java.lang.Byte、java.lang.Short、java.lang.Integer、
     * java.lang.Long 和 java.lang.Character 。如果满足这个条件，则返回
     * true。
     * </li>
     * <li>不满足上述所有条件，则返回 false。</li>
     * </ol>
     * </p>
     *
     * @param clazz 目标类型，如果是 null 总是返回 false
    ...
    protected static class TypeInfo {
        ...
        /**
         * 取得 TypeInfo 所代表的java类。
         * @return TypeInfo 所代表的java类
         */
    ...
    //XXX:有关primitive类型的方法。

    /**
     * 返回指定类型所对应的primitive类型。
     *
     * @param clazz 要检查的类型
     * @return 如果指定类型为null或不是primitive类型的包装类，则返回null，否则返回相应的primitive类型。
     */
    @SuppressWarnings("rawtypes")
     ...
     *
     * @param clazz 要检查的类型
     * @return 如果指定类型为null，则返回null，如果是primitive类型，则返回相应的包装类，否则返回原始的类型。
     */
    @SuppressWarnings("rawtypes")
